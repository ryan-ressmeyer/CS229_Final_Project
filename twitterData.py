...
new_tweets = api.user_timeline(screen_name = 'JoeBiden',count=200)
...
while len(new_tweets) > 0 and len(alltweets) < 10000:
	...
	new_tweets = api.user_timeline(screen_name = 'JoeBiden',count=200,max_id=oldest)
...
a = np.repeat("Joe Biden", len(lastLast))
